...
type DepositStatus = "Succeeded" | "Pending" | "Failed";
...
function getStatusStyles(status: DepositStatus) {
  switch (status) {
    case "Pending":
      return {
        color: "#FF9900",
...
export default function BybitDepositPage() {
  const receiptRef = useRef<HTMLDivElement>(null);

  const [coin, setCoin] = useState("USDT");
  const [amount, setAmount] = useState("1,161.5");
  const [depositAccount, setDepositAccount] = useState("Funding Account");
  const [chainType, setChainType] = useState("TRON (TRC20)");
  const [address, setAddress] = useState(
    "TPuqCrcBvJSoJBRKzNqmmmcXEwbJGEWx",
  );
  const [txid, setTxid] = useState(
    "facf8c80f170ec0355068c5f092eb9004f25f67d834b991b505d73220dda4b8a",
  );
  const [time, setTime] = useState("2026-08-05 07:28:39");
  const [status, setStatus] = useState<DepositStatus>("Succeeded");
  ...
  async function exportReceipt(action: "download" | "copy") {
    ...
    if (action === "download") {
      ...
      try {
        ...
        link.download = `bybit-deposit-${Date.now()}.png`;
        link.href = dataUrl;
        link.click();
      } finally {
        setExporting(false);
  ...
  return (
    <div className="min-h-screen bg-[#eef0f3] text-[#1f2328]">
      ...
      <main className="mx-auto grid max-w-[1400px] grid-cols-1 gap-6 px-4 py-6 xl:grid-cols-[320px_minmax(0,1fr)_300px]">
        {/* Left — Bybit Deposit Settings */}
        <section className="rounded-2xl bg-white p-5 shadow-sm">
          <h2 className="mb-5 text-lg font-semibold">Bybit Deposit Settings</h2>
            ...
            >
              ...
              <div className="flex min-h-0 flex-1 flex-col overflow-hidden pb-10">
                ...
                <div className="flex flex-col gap-[15px]">
                  <DetailRow label="Deposit Account" value={depositAccount} />
