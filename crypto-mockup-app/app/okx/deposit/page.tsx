...
const labelClass =
  "shrink-0 text-[13px] font-semibold tracking-normal text-white";
const valueClass =
  "text-[13px] font-normal tracking-normal text-[#8E8E93]";

export default function OkxDepositPage() {
  ...
  const [address, setAddress] = useState(
    "TJdvfHYru5GEYg9sGN4Nco1q87BVk2nb3m",
  );
  const [txid, setTxid] = useState(
    "ce58a3f120b65f7466e2f6ae8ff3fdf22136ffb7b7eb3b0bdbe18d5ca19063d3",
  );
  const [time, setTime] = useState("2026-09-02 21:43:00");
  ...
  async function exportReceipt(action: "download" | "copy") {
    ...
    if (action === "download") {
      ...
      try {
        ...
        link.download = `okx-deposit-${Date.now()}.png`;
        link.href = dataUrl;
        link.click();
      } finally {
        setExporting(false);
    ...
    <div className="min-h-screen bg-[#eef0f3] font-okx text-[#1f2328]">
      ...
      <main className="mx-auto grid max-w-[1400px] grid-cols-1 gap-6 px-4 py-6 xl:grid-cols-[320px_minmax(0,1fr)_300px]">
        <section className="rounded-2xl bg-white p-5 shadow-sm">
          <h2 className="mb-5 text-lg font-semibold">OKX Deposit Settings</h2>
          <div className="space-y-4">
            ...
            <Field label="Tron TRC20 Address">
              <input
                value={address}
              ...
              <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-5 pb-10">
                ...
                <div className="mt-0 space-y-0">
                  <div className="flex items-center justify-between gap-3 py-[18px]">
                    <span
                      className={`inline-flex items-center gap-1 ${labelClass}`}
                    >
                      Price
                      <Info
                    ...
                    <span className={valueClass}>${price}/USDT</span>
                  </div>

                  <div className="flex items-center justify-between gap-3 py-[18px]">
                    <span className={labelClass}>Network</span>
                    <span
                      className={`inline-flex items-center gap-1.5 ${valueClass}`}
                    >
                      <TronDot />
                      Tron (TRC20)
                    </span>
                  ...
                  <div className="flex items-start justify-between gap-3 py-[18px]">
                    ...
                    <div className="flex min-w-0 items-start gap-1.5">
                      <div
                        className={`max-w-[190px] break-all text-right leading-[1.35] ${valueClass}`}
                      >
                        {address}
                      </div>
                      <Copy
                        className="mt-0.5 h-3.5 w-3.5 shrink-0 cursor-pointer text-[#8E8E93]"
                ...
                <div className="mt-auto flex flex-col items-center pt-6 pb-2">
                  <button
                    type="button"
                    className="w-full rounded-full bg-[#BBFF2E] py-3.5 text-center text-[15px] font-bold text-black transition-colors hover:bg-[#b2da2f]"
                  >
                    View in Blockchain Browser
                  </button>
                </div>
              </div>
            </div>
